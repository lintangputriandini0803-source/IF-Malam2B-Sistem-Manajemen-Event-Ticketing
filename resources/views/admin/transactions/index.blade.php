...
@section('page-sub', 'Laporan & Rekonsiliasi Pembayaran')
...
<div class="stat-grid">
    ...
    <div class="sc c-g">
        ...
        <div class="split">
            <div class="sp-item"><span class="sp-num clr-g">{{ $totalSuccess }}</span><span class="sp-lbl">Berhasil</span></div>
            <div class="sp-item"><span class="sp-num clr-w">{{ $totalPending }}</span><span class="sp-lbl">Pending</span></div>
            <div class="sp-item"><span class="sp-num clr-r">{{ $totalFailed }}</span><span class="sp-lbl">Batal</span></div>
        </div>
        <div class="sc-sub" style="margin-top:8px">Total: {{ $totalSuccess+$totalPending+$totalFailed }} transaksi</div>
    </div>
    <div class="sc c-b">
        <div class="sc-ico">🏦</div>
        <div class="sc-lbl">Revenue Platform (Fee)</div>
        <div class="sc-val" style="font-size:18px;color:var(--bl)">Rp{{ number_format($totalRevenue,0,',','.') }}</div>
        <div class="sc-sub">{{ $totalSuccess }} tiket × Rp2.000</div>
    </div>
    <div class="sc c-o">
        <div class="sc-ico">🏷️</div>
        <div class="sc-lbl">Dana ke Panitia (Estimasi)</div>
        <div class="sc-val" style="font-size:18px;color:var(--ok)">Rp{{ number_format($totalPayout,0,',','.') }}</div>
        ...
        <div class="fbar-inner">
            ...
            <div class="fg">
                ...
                <select name="payment_method">
                    ...
                    <option value="bca_va"      {{ request('payment_method')=='bca_va'?'selected':'' }}>🏦 BCA Virtual Account</option>
                    <option value="mandiri_va"  {{ request('payment_method')=='mandiri_va'?'selected':'' }}>🏦 Mandiri Virtual Account</option>
                    <option value="bni_va"      {{ request('payment_method')=='bni_va'?'selected':'' }}>🏦 BNI Virtual Account</option>
                    <option value="bri_va"      {{ request('payment_method')=='bri_va'?'selected':'' }}>🏦 BRI Virtual Account</option>
                    <option value="permata_va"  {{ request('payment_method')=='permata_va'?'selected':'' }}>🏦 Permata Virtual Account</option>
                </select>
            </div>
            <button type="submit" class="btn-f">Terapkan</button>
            @if(request()->hasAny(['search','date_from','date_to','status','event_id','payment_method']))
...
<div class="tcard">
    <div class="tcard-head">
        <div>
            ...
            <div class="tc-count">
                @if(method_exists($transactions,'total'))
                    ...
                    @if($transactions->lastPage()>1)· Hal {{ $transactions->currentPage() }}/{{ $transactions->lastPage() }}@endif
                @else
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap">
            @php $today=now()->toDateString();$week=now()->startOfWeek()->toDateString();$month=now()->startOfMonth()->toDateString(); @endphp
            <a href="{{ route('admin.transactions.index', array_merge(request()->except(['date_from','date_to']),['date_from'=>$today,'date_to'=>$today])) }}" class="btn-sm">Hari ini</a>
            <a href="{{ route('admin.transactions.index', array_merge(request()->except(['date_from','date_to']),['date_from'=>$week,'date_to'=>$today])) }}" class="btn-sm">Minggu ini</a>
            <a href="{{ route('admin.transactions.index', array_merge(request()->except(['date_from','date_to']),['date_from'=>$month,'date_to'=>$today])) }}" class="btn-sm">Bulan ini</a>
        </div>
    </div>
        ...
        <table class="tt">
            ...
            @php
                ...
                [$mClass,$mIcon,$mLabel] = match($payMethod){
                    'bca_va'     =>['b-bank','🏦','BCA Virtual Account'],
                    'mandiri_va' =>['b-bank','🏦','Mandiri Virtual Account'],
                    'bni_va'     =>['b-bank','🏦','BNI Virtual Account'],
                    'bri_va'     =>['b-bank','🏦','BRI Virtual Account'],
                    'permata_va' =>['b-bank','🏦','Permata Virtual Account'],
            ...
            <tr>
                <td>
                    ...
                    @if($tx->order_ref)<div class="tx-ref">Ref: {{ $tx->order_ref }}</div>@endif
                </td>
                <td>
                    <div class="by-name">{{ $tx->name }}</div>
                    <div class="by-ct">{{ $tx->email }}</div>
                    @if($tx->phone)<div class="by-ct">📱 {{ $tx->phone }}</div>@endif
                    @if($tx->nim)<div class="by-ct">NIM: {{ $tx->nim }}</div>@endif
                </td>
                <td>
                ...
                <td>
                    <div style="font-size:13px;font-weight:600;color:var(--g900)">{{ optional($ticketType)->name ?? '—' }}</div>
                    <div style="font-size:11px;color:var(--g400);margin-top:2px">× {{ $tx->quantity }} tiket</div>
                </td>
                <td>
                    <span class="badge {{ $mClass }}">{{ $mIcon }} {{ $mLabel }}</span>
                    @if($tx->virtual_account)
                    <div style="font-size:10.5px;color:var(--g400);margin-top:3px;font-family:monospace">VA: {{ $tx->virtual_account }}</div>
                    @endif
                </td>
                ...
                <td>
                    ...
                    @if($fee>0)<div class="tx-fee">Fee: Rp{{ number_format($fee,0,',','.') }}</div>@endif
                </td>
                <td>
                    @php $proof = $tx->payment_proof ?? null; @endphp
                    @if($proof)
                        <a href="#" class="btn-sm" onclick="openProof('{{ asset('storage/'.$proof) }}','{{ $tx->reg_number }}');return false;">🖼 Lihat</a>
...
<div class="recon">
    <div class="recon-head">
        <div>
            ...
            <div class="rh-sub">Ringkasan keuangan berdasarkan filter aktif — untuk audit &amp; transfer ke panitia</div>
        </div>
        <a href="{{ route('admin.transactions.index', array_merge(request()->query(),['export'=>'1'])) }}"
           class="btn btn-outline" style="font-size:12px;padding:7px 13px">
    ...
    <div class="prow">
        ...
        <div class="pi">
            <div class="pi-lbl">Potongan Fee Platform</div>
            <div class="pi-val cb">Rp{{ number_format($totalRevenue,0,',','.') }}</div>
            <div class="pi-sub">{{ $totalSuccess }} tiket × Rp2.000/tiket</div>
        </div>
        <div class="pi">
        ...
        <div class="pi">
            ...
            <div class="split-sm">
                <div class="ss-item"><div class="n clr-g">{{ $totalSuccess }}</div><div class="l">Berhasil</div></div>
                <div class="ss-item"><div class="n clr-w">{{ $totalPending }}</div><div class="l">Pending</div></div>
                <div class="ss-item"><div class="n clr-r">{{ $totalFailed }}</div><div class="l">Batal</div></div>
            </div>
            @php $tot=$totalSuccess+$totalPending+$totalFailed; @endphp
            <div class="pi-sub" style="margin-top:6px">Konversi: {{ $tot>0?round(($totalSuccess/$tot)*100):0 }}% sukses</div>
        </div>
    </div>
</div>
