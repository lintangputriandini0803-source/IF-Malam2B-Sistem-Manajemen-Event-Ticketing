...
@section('content')
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
        ...
        <div style="display:flex;align-items:center;gap:12px">
            ...
            <button onclick="document.getElementById('modalTambahPanitia').style.display='flex'"
                style="display:flex;align-items:center;gap:7px;padding:9px 18px;background:#6B0080;color:white;border:none;border-radius:10px;font-size:13px;font-weight:700;cursor:pointer"
    ...
    <div style="background:white;border-radius:14px;padding:16px 20px;margin-bottom:16px">
        <form method="GET" action="{{ route('admin.users.index') }}"
            ...
            <div
                style="display:flex;flex:1;min-width:240px;border:1.5px solid #e5e7eb;border-radius:10px;overflow:hidden;background:white">
                <div style="position:relative;border-right:1.5px solid #e5e7eb;flex-shrink:0">
                    ...
                    <svg style="position:absolute;right:10px;top:50%;transform:translateY(-50%);width:14px;height:14px;color:#9ca3af;pointer-events:none"
    ...
    <div style="background:white;border-radius:14px;overflow:hidden">
        <table style="width:100%;border-collapse:collapse">
            <thead style="background:#f8fafc">
                <tr>
                    <th
                        style="text-align:left;padding:14px 20px;font-size:11px;font-weight:700;color:#9ca3af;text-transform:uppercase;letter-spacing:.05em">
                        Pengguna</th>
                    <th
                        style="text-align:left;padding:14px 20px;font-size:11px;font-weight:700;color:#9ca3af;text-transform:uppercase;letter-spacing:.05em">
                        Role</th>
                    ...
                    <th
                        ...
                        Bergabung</th>
                    <th
                        style="text-align:right;padding:14px 20px;font-size:11px;font-weight:700;color:#9ca3af;text-transform:uppercase;letter-spacing:.05em">
                        Aksi</th>
                </tr>
            </thead>
            <tbody>
                @forelse($users ?? [] as $user)
                    <tr style="border-top:1px solid #f3f4f6;transition:background .1s"
                        onmouseover="this.style.background='#fafafa'" onmouseout="this.style.background='white'">
                        <td style="padding:14px 20px">
                            <div style="display:flex;align-items:center;gap:12px">
                                <div
                                    style="width:38px;height:38px;border-radius:50%;background:#6B0080;display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:700;color:white;flex-shrink:0">
                        ...
                        <td style="padding:14px 20px;text-align:right">
                            <div class="dropdown" style="display:inline-block">
                                <button data-dropdown
                                    ...
                                    <svg style="width:15px;height:15px;color:#6b7280" fill="none" viewBox="0 0 24 24"
                                        stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <div class="dropdown-menu">
                                    ...
                                    @elseif($user->role === 'panitia' && $user->status === 'approved')
                                        <form method="POST" action="{{ route('admin.users.reject', $user->id) }}">
                                            @csrf @method('PATCH')
                                            <button type="submit" class="dropdown-item danger w-full text-left">❌ Cabut
                                                Akses</button>
                                        </form>
                                    @elseif($user->role === 'panitia' && $user->status === 'rejected')
                                        <form method="POST" action="{{ route('admin.users.approve', $user->id) }}">
                                            @csrf @method('PATCH')
                                            <button type="submit" class="dropdown-item w-full text-left">✅ Izinkan Kembali</button>
                                        </form>
                                    @else
    ...
    <div id="modalTambahPanitia"
        ...
        <div
            style="background:white;border-radius:16px;width:100%;max-width:480px;margin:16px;box-shadow:0 20px 60px rgba(0,0,0,.2);animation:slideUp .2s ease">

            {{-- Header Modal --}}
            <div
                style="display:flex;align-items:center;justify-content:space-between;padding:20px 24px;border-bottom:1px solid #f3f4f6">
                <div style="display:flex;align-items:center;gap:10px">
                    <div
                        style="width:36px;height:36px;border-radius:10px;background:#f5eeff;display:flex;align-items:center;justify-content:center">
                        <svg style="width:18px;height:18px;color:#6B0080" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div>
                        <h2 style="font-size:15px;font-weight:800;color:#111;margin:0">Tambah Panitia</h2>
                        <p style="font-size:12px;color:#9ca3af;margin:0">Buat akun panitia baru</p>
                    </div>
                </div>
                <button onclick="document.getElementById('modalTambahPanitia').style.display='none'"
            ...
            <form method="POST" action="{{ route('admin.users.store') }}" style="padding:24px">
                ...
                <div style="margin-bottom:16px">
                    ...
                    <div style="position:relative">
                        <select name="role" required id="selectRoleModal" onchange="toggleOrganisasiField(this.value)"
                        ...
                        <svg style="position:absolute;right:12px;top:50%;transform:translateY(-50%);width:14px;height:14px;color:#9ca3af;pointer-events:none"
                            fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                        </svg>
                    </div>
                ...
                <div style="margin-bottom:16px">
                    ...
                    <div style="position:relative">
                        ...
                        <button type="button" onclick="togglePassword()"
                            ...
                            <svg id="eyeIcon" style="width:16px;height:16px" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div style="display:flex;gap:10px">
                    <button type="button" onclick="document.getElementById('modalTambahPanita').style.display='none'"
